...
def main():
    ...
    print()
    print('  This ARN was extracted from the ${arn:...} wrapper in the headers dict')
    print('  by resolve_header_references(), then passed to resolve_credential_arn().')
    print('  It identifies which vault secret to fetch.')

    print()
    print('  --- The code that extracts the ARN ---')
    print('  loopy/tools/tool_provider.py:')
    show_function(tools_source, 'resolve_header_references')
    print()
    print('  The ARN we found in Stage 1 likely first appears at:')
    print('    arn = match.group(1)    # <-- regex strips ${...}, extracts bare ARN')
    print()
    print('  --- Where the ARN gets resolved to a JWT ---')
    print('  loopy/util/clients.py:')
    ...
    print()
    print('  The JWT we find in Stage 2 likely first appears at:')
    print('    return api_key          # <-- plain string returned to caller')
    print()
    print('  --- The call site (where everything connects) ---')
